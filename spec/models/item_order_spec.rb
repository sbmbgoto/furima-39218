  ...
  describe '購入フォーム' do
    ...
    context '内容に問題がある場合' do
      ...
      it "phone_numberが9桁以下では登録できない" do
        ...
        @item_order.valid?
        expect(@item_order.errors.full_messages).to include("Phone number is too short (minimum is 10 characters)")
      end
      ...
      it "phone_numberが全角数字では登録できない" do
        ...
        @item_order.valid?
        expect(@item_order.errors.full_messages).to include("Phone number は10～11桁の半角数字で入力してください")
      end
      ...
      it "post_codeにハイフンが含まれていないと登録できない" do
        ...
        @item_order.valid?
        expect(@item_order.errors.full_messages).to include("Post code はハイフンを含めた半角数字で入力してください")
      end
